...
Vehicle.prototype.eat = function (list, index) {
  ...
  for (var i = list.length - 1; i >= 0; i--) {
    // Calculate distance
    var d = dist(list[i].x, list[i].y, this.position.x, this.position.y);

    // If it's within perception radius and closer than pervious
    if (d < this.dna[2 + index] && d < closestD && list[i].highlight === true) {
      closestD = d;
      // Save it
      closest = createVector(list[i].x, list[i].y);

      // If we're withing 5 pixels, eat it!
      if (d < 5) {
        // Add or subtract from health based on kind of food
        //this.health += nutrition[index];
        this.health += list[i].r;
        list.splice(i, 1);
        return i;
      }
    }
